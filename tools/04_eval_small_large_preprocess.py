...
# -----------------------
# PATHS (Preprocessing run predictions)
# -----------------------
...
# This should point to the TEST predictions of the preprocessing experiment
PRED_LABELS_DIR = "/home/linuxu/Desktop/Research_Project/yolov7/runs/test/seadronessee_tiled_test/labels"

OUT_DIR = "/home/linuxu/Desktop/Research_Project/yolov7/small_large_eval_preprocess"
os.makedirs(OUT_DIR, exist_ok=True)

# -----------------------
# CLASSES (must match your YAML order)
# -----------------------
...
def main():
    ...
    # Confusion matrix for groups: rows=true, cols=pred
    # 0=Small, 1=Large
    ...
    for pf in pred_files:
        ...
        if not pred_raw:
            # no predictions -> all GT are FN by their group
        ...
        # Build GT boxes and Pred boxes (absolute xyxy) + group labels
        gts = []
        ...
        # Greedy IoU matching between preds and gts
        if len(preds) == 0:
            for _, g_true in gts:
                fn[g_true] += 1
            continue
        ...
        while True:
            ...
            if g_pred == g_true:
                ...
                # group mismatch -> counts as FP for predicted group and FN for true group
                fp[g_pred] += 1
                fn[g_true] += 1

            iou_mat[i, j] = -1.0

        # Unmatched predictions -> FP by their group
        for i, (_, g_pred) in enumerate(preds):
            if i not in matched_pred:
                fp[g_pred] += 1

        # Unmatched GT -> FN by their group
    ...
    for g in [0, 1]:
        print(
            f"{labels[g]}: TP={tp[g]} FP={fp[g]} FN={fn[g]} "
            f"P={prec[g]:.4f} R={rec[g]:.4f} F1={f1[g]:.4f}"
        )
